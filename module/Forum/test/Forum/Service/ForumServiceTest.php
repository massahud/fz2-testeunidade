...
class ForumServiceTest extends PHPUnit_Framework_TestCase {
     ...
     * @test
     */
    public function deveChamarRemoveAoApagarTopico() {
        /* @var $em \Doctrine\ORM\EntityManager */
        $em = Phake::mock('Doctrine\ORM\EntityManager');
        $topico = Phake::mock('Forum\Model\Entidade\Topico');
        Phake::when($em)->find('Forum\Model\Entidade\Topico', self::UM_ID)->thenReturn($topico);
        $timeService = Phake::mock('Forum\Service\TimeService');

        $service = new ForumService($em, $timeService);

        $ok = $service->apagarTopico(self::UM_ID);

        Phake::inOrder(
                Phake::verify($em)->remove($topico), 
                Phake::verify($em)->flush($topico)
        );
        expect($ok)->true();
    }
